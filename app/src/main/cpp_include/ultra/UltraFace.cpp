...
UltraFace::UltraFace(const std::string &bin_path, const std::string &param_path,
    ...
    ultraface.opt.lightmode = true;
    ultraface.opt.use_vulkan_compute = false;
    ultraface.opt.blob_allocator = &g_blob_pool_allocator;
    ultraface.opt.workspace_allocator = &g_workspace_pool_allocator;
    if (ultraface.opt.use_vulkan_compute) {
...
int UltraFace::detect(ncnn::Mat &img, std::vector<FaceInfo> &face_list) {
    ...
    if (ncnn::get_gpu_count() > 0 && ultraface.opt.use_vulkan_compute) {
...
void UltraFace::generateBBox(std::vector<FaceInfo> &bbox_collection, ncnn::Mat scores, ncnn::Mat boxes,
                             float _score_threshold, int _num_anchors) {
    for (int i = 0; i < _num_anchors; i++) {
        if (scores.channel(0)[i * 2 + 1] > _score_threshold) {
            FaceInfo rects;
    ...
    for (int i = 0; i < box_num; i++) {
        ...
        switch (type) {
            ...
            case blending_nms: {
                ...
                for (auto & j : buf) {
                    float rate = exp(j.score) / total;
                    rects.x1 += j.x1 * rate;
                    rects.y1 += j.y1 * rate;
                    rects.x2 += j.x2 * rate;
                    rects.y2 += j.y2 * rate;
                    rects.score += j.score * rate;
                }
